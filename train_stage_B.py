...
def train(gpu_id):
    ...
    for it in pbar:
        ...
        with torch.cuda.amp.autocast(dtype=_float16_dtype), torch.no_grad():
            ...
            noised_latents, mask = model.generator.add_noise(latents, t)
            loss_weight = model.generator.get_loss_weight(t, mask)
        ...
        if it == 1 or it % args.print_every == 0 or it == args.max_iters:
            ...
            if it % args.extra_ckpt_every == 0:
                torch.save({
                    ...
                    'effnet_state_dict': model.effnet.state_dict(),
            ...
            with torch.no_grad():
                ...
                # Efficientnet stuff
                effnet_embeddings = model.effnet(effnet_preprocess(images))
                ...
                latents = vqmodel.encode(images)[2]
                noised_latents, mask = model.generator.add_noise(latents, t)
                pred = model.generator(noised_latents, t, effnet_embeddings, clip_text_embeddings)
                ...
                sampled = sample(model.generator, {'effnet': effnet_embeddings, 'byt5': clip_text_embeddings},
                ...
                sampled_noimg = sample(model.generator,
